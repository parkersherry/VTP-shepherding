...
% set first half of indices to 2
ids(1:N/2) = 2;
...
% save in guy
DT_t{t} = DT;
% save current displacement (unit?) vectors in guy at time t
U_t(:,:,t) = U;
[nbhd, nearest, d] = neighborhoods(DT);
fun = @(x) transition(x,'expReciprocal');

% array of sigma applied to delta_i/L
s = arrayfun(fun, d/L);

% repulsion
r = X - X(nearest,:);   % vector from agent to nearest neighbour
rnorm = vecnorm(r,2,2); % Euclidean norm in R^2
